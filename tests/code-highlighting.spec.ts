...
test.describe("Code Block Highlighting", () => {
  ...
  test("should have syntax highlighted code blocks with proper span tags", async ({
    page,
  }) => {
    ...
    if (codeBlockCount > 0) {
      ...
      const spanElements = firstCodeBlock.locator(
  ...
  test("should have different colors for different syntax elements", async ({
    page,
  }) => {
    ...
    if ((await codeBlocks.count()) > 0) {
      ...
      const uniqueColors = await firstCodeBlock
  ...
  test("should preserve code block structure", async ({ page }) => {
    ...
    if ((await codeBlocks.count()) > 0) {
      ...
      expect(html).toMatch(
        /<span class="line"><span style="color:#[0-9A-Fa-f]{6}">/
      );
